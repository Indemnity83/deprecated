		...
		<div class="col-md-12">
			<div class="page-header">
				<ul class="nav nav-pills pull-right">
					<li><?php echo $this->Html->link(__('<span class="glyphicon glyphicon-edit"></span>&nbsp&nbsp;Edit Role'), array('action' => 'edit', $role['Role']['slug']), array('escape' => false)); ?> </li>
...
<div class="related row">
	...
	<?php if (!empty($role['User'])): ?>
		...
		<?php foreach ($role['User'] as $user): ?>
			<tr>
				<td><?php echo $this->Html->link($user['username'], array('controller' => 'users', 'action' => 'view', $user['username'])); ?></td>
				<td><?php echo $this->Html->link($user['email'], 'mailto:' . $user['email']); ?></td>
				<td class="actions text-right">
					<?php echo $this->Html->link(__('<span class="glyphicon glyphicon-search"></span>'), array('controller' => 'users', 'action' => 'view', $user['username']), array('escape' => false)); ?>
					<?php echo $this->Html->link(__('<span class="glyphicon glyphicon-edit"></span>'), array('controller' => 'users', 'action' => 'edit', $user['username']), array('escape' => false)); ?>
